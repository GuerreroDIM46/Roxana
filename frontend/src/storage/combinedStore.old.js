...
import { getListados, getElementosListado, postElemento, patchElemento, postListado } from '@/storage/LlamadasAPI'; // Importar las llamadas centralizadas
...
export const useCombinedStore = defineStore('CombinedStore', {
    ...
    actions: {
        ...
        async verificarElementosModificados() {
            try {
                ...
                if (this.alertaSobreescritura !== 'ignorar') {
                    // Buscar elementos en DexieDB con flags "creado" o "modificado"
                    const elementosModificados = await db.elementos
                        .where('flag')
                        .anyOf('creado', 'modificado')
                        .toArray();
        
                    // Buscar listados en DexieDB con flags "creado" o "modificado"
                    const listadosModificados = await db.listados
                        .where('flag')
                        .anyOf('creado', 'modificado')
                        .toArray();
        
                    // Si hay elementos o listados modificados o creados, actualizar alertaSobreescritura
                    if ((elementosModificados.length + listadosModificados.length) > 0) {
        ...
        async cargarElementos(listadoId) {
            // Verificar si hay elementos o listados modificados antes de continuar
            const puedeContinuar = await this.verificarElementosModificados()
            if (!puedeContinuar) {
                console.warn('Operación interrumpida debido a alerta de sobreescritura');
            ...
            try {
                const response = await getElementosListado(listadoId);
